...
select Movie_Name,budget from movies_sql where revenue-budget;

#8.	Retrieve the names and profit of all the Bollywood movies in the dataset.(profit=revenue – budget) 
select Movie_Name,revenue,budget from movies_sql where budget-revenue;
...
select movie_name,lead_star from movies_sql where genre='comedy' and lead_star='akshay kumar';
...
select movie_name,revenue,budget from movies_sql where lead_star='Salman khan' order by revenue desc limit 5;
#24.	Retrieve the names of top 5 floped movies in the dataset.
select movie_name,revenue,budget from movies_sql where revenue<budget limit 5; 
#25.	Retrieve the names of top 5 hit movies in the dataset. 
select movie_name,revenue,budget from movies_sql where budget<revenue limit 5; 
...
select movie_name from movies_sql where lead_star='Amitabh bachchan'order by budget desc limit 1 offset 1;
#29.	Which are the flopped movies of Akshay Kumar. 
select movie_name,budget,revenue from movies_sql where budget>revenue and lead_star='akshay kumar';
#30.	With which director Sharukh Khan have given the biggest hit movie .
select movie_name,director,revenue,budget from movies_sql where lead_star='shahrukh khan'order by revenue desc limit 1;  
